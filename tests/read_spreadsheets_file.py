...
def generate_testrail_csv(file_key: str):
    gc = pygsheets.authorize(client_secret=f"{AppFolders.RESOURCES_PATH}/client_secret.json",
                             service_account_file=f"{AppFolders.RESOURCES_PATH}/secret-key.json")
    columns = ['Title', 'Section', "References", 'Preconditions', "Steps", "Expected Result", "Priority"]
            ...
            if "TC ID" in values:
                ...
                for row in values:
                    if row[7] != "" and "Precondition" != row[7]:
                        ...
                        if row[6]:
                            sub_category3 = row[6]
                        data = [f"[{category} {sub_category1} {sub_category2}] - {sub_category3}", section, link,
                                f"{row[7]}", f"{row[8]}", f"{row[9]}", f"{row[10] if row[10] else "Medium"}"]
