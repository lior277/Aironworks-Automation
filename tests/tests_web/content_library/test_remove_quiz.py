...
from src.models.factories.scenario.clone_education_content_model_factory import CloneEducationContentModelFactory
...
def cloned_education_content(request, api_request_context_aw_admin, user: UserModel,
                             api_request_context_customer_admin) -> Item:
    def finalizer():
        expect(education.delete_education_content(cloned_content.id)).to_be_ok()

    request.addfinalizer(finalizer)
    request_context = api_request_context_aw_admin if user.is_admin else api_request_context_customer_admin
    ...
    education_content_to_clone = CloneEducationContentModelFactory.get_education_content(education_content_details)
...
@pytest.mark.smoke
def test_remove_quiz(user: UserModel, cloned_education_content: Item,
