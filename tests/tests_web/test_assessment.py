...
def test_report_can_be_resolved(user, api_request_context_addin, employee_reports_page):
    ...
    response = assessment_service.assessment_report(message, "random@mail.com", subject=message)
    ...
    last_status = wait_for_lro(lambda: assessment_service.assessment_report_by_id(id), 60)
...
def test_assessment_outlook(outlook_page):
    ...
    expect(outlook_page.app_frame.get_by_text("The email was sent to your")).to_be_visible(timeout=60 * 1000)
