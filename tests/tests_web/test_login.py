...
@pytest.mark.parametrize("user,error_message", [(UserModelFactory.user(), 'Invalid Email address or password')])
@pytest.mark.web
def test_login_error_message(sign_in_page: SignInPage, user, error_message):
    sign_in_page.navigate()
    sign_in_page.submit_sing_in_form(user)
    expect(sign_in_page.page.locator(BasePageLocators.MUI_ALERT_MESSAGE)).to_have_text(error_message)


@pytest.mark.parametrize("user", [UserModelFactory.my_user()])
def test_login_existing_user(sign_in_page: SignInPage, dashboard_page: DashboardPage, user):
    sign_in_page.navigate()
    sign_in_page.submit_sing_in_form(user)
