...
class EmployeeModelFactory:
    ...
    @staticmethod
    def get_random_employees(count: int, email_name: str = None, domain: str = None,
                             mailtrap_inbox: str = None) -> list[EmployeeModel]:
        emails = EmployeeModelFactory.get_random_emails(count, email_name, domain, mailtrap_inbox=mailtrap_inbox)
        return [EmployeeModelFactory.get_random_employee(email) for email in emails]

    @staticmethod
    def get_random_emails(count: int, email_name: str = None, domain: str = None, mailtrap_inbox: str = None) -> list[str]:
        emails = set()
