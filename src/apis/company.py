...
class CompanyService(BaseService):
    ...
    def create_employees(self, file_path: str, overwrite: bool = False) -> APIResponse:
        return self._post(PSApi.UPLOAD_EMPLOYEE_INFO.get_endpoint(),
    ...
    def create_employees_status(self, op_id: str) -> APIResponse:
        return self._get(PSApi.UPLOAD_EMPLOYEE_INFO_STATUS.get_endpoint().format(op_id=op_id))
    ...
    def patch_localized_config(self, language: str,
                               localized_configs_model: PatchLocalizedConfigsModel) -> APIResponse:
        return self._patch(PSApi.COMPANY_LOCALIZED_CONFIG_LANGUAGE.get_endpoint().format(language=language),
    ...
    def employee_by_mail(self, email: str) -> APIResponse:
        result = self._post(PSApi.EMPLOYEE_LIST.get_endpoint(),
                            data={"employee_role": True,
                                  "filters": {
                                      "items": [
                                          {
                                              "columnField": "email",
    ...
    def get_employee_ids(self, employee_ids: EmployeeListIdsModel) -> APIResponse:
        return self._post(PSApi.EMPLOYEE_LIST_IDS.get_endpoint(), data=asdict(employee_ids))
