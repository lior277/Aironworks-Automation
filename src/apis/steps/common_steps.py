...
def run_education_campaign_on_employee(request_context: APIRequestContext, mailtrap, employee):
    education = api.education(request_context)
    result = education.start_campaign(EducationCampaignModel(
        title="Automation Campaign " + datetime.now().strftime("%d/%m/%Y, %H:%M:%S"),
        start_date=datetime.now().timestamp(), end_date=(datetime.now() + timedelta(days=1)).timestamp(),
        employee_ids=[employee.employee_id], content_id=AppConfigs.EXAMPLE_EDUCATION_CONTENT))
    expect(result).to_be_ok()

    assert "id" in result.json()
    mail = mailtrap.wait_for_mail(AppConfigs.EMPLOYEE_INBOX_ID, find_email(employee.email))
    return mail


@allure.step("create employees")
def create_employees(request_context: APIRequestContext, employees: list[EmployeeModel],
    ...
    path = api.upload(request_context).upload_file("employees.csv", "text/csv", data, "TEMPORARY")
...
def create_employees_wait(request_context: APIRequestContext, employees: list[EmployeeModel],
                          overwrite: bool = False) -> APIResponse:
...
def create_employee(request_context: APIRequestContext, employee: EmployeeModel) -> APIResponse:
