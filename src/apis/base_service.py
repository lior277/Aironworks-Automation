...
def allure_attach(response: APIResponse):
    allure.attach(body=json.dumps(response.json(), indent=4), attachment_type=allure.attachment_type.JSON,
                  name="response")
...
class BaseService:
    ...
    def _post(self, url, data=None, params=None, timeout=None) -> APIResponse:
        response = self._request.post(url=url, data=data, params=params, timeout=timeout)
