...
class UploadService(BaseService):
    ...
    def upload_file_api(self, path: str, name: str, kind: Literal["LOGO", "ATTACHMENT", "TEMPORARY"], ) -> APIResponse:
        return self._post(PSApi.UPLOAD_URL.get_endpoint(), data={"path": path, "name": name, "kind": kind})

    @allure.step("UploadService: upload file api")
    def upload_file(self, path: str, type: str, data: str,
                    kind: Literal["LOGO", "ATTACHMENT", "TEMPORARY"]) -> APIResponse:
        name = os.path.basename(path)
        response = self._post(PSApi.UPLOAD_URL.get_endpoint(), data={"name": name, "kind": kind})
