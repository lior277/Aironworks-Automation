...
class SurveyService(BaseService):
    ...
    def add_survey(self, survey: AddSurveyModel) -> APIResponse:
        return self._post(PSApi.ADD_SURVEY.get_endpoint(), data=survey.to_filtered_dict())

    @allure.step("SurveyService: set default survey {survey_id}")
    def set_default_survey(self, survey_id: AddSurveyModel, value: bool = True) -> APIResponse:
    ...
    def delete_survey(self, survey_id: str) -> APIResponse:
        return self._post(PSApi.DELETE_SURVEY.get_endpoint(), data={"survey_id": f"{survey_id}"})
    ...
    def get_specific_survey_answer_stats(self, survey_id: str) -> APIResponse:
        return self._get(PSApi.GET_SPECIFIC_SURVEY_ANSWER_STATS.get_endpoint(),
