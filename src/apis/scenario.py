...
class ScenarioService:
    ...
    def post_list_attack_infos(cls, request_context: APIRequestContext,
                               list_attack_infos: ListAttackInfosModel) -> APIResponse:
        return request_context.post(PSApi.LIST_ATTACK_INFOS.get_endpoint(), data=asdict(list_attack_infos))

    @classmethod
    @allure.step("ScenarioService: get attack infos")
    def get_attack_info(cls, request_context: APIRequestContext, scenario_id: str) -> APIResponse:
        return request_context.get(PSApi.GET_ATTACK_INFO.get_endpoint(), params={"id": f"{scenario_id}"})

    @classmethod
    @allure.step("ScenarioService: aw admin get campaign urls")
    def aw_admin_campaign_urls(cls, request_context: APIRequestContext, campaign_id: int,
                               wait_time: int = 100) -> CampaignUrls:
        start_time = datetime.now()
        while True:
            response = request_context.get(PSApi.ADMIN_CAMPAIGN_ATTACK_URLS.get_endpoint()
