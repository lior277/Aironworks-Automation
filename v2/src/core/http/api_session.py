...
FormData = dict[
    str, tuple
]  # {"file": ("name.pdf", fileobj_or_bytes, "application/pdf")}
...
class ApiSession:
    ...
    def post_form(
        self, url: str, files: FormData, data: dict | None = None
    ) -> requests.Response:
        """
    ...
    def _request(
        self, method: str, url: str, json_body: JsonBody = None
    ) -> requests.Response:
        full_url = self._full_url(url)

        request_context = RequestContext(method=method, url=full_url, body=json_body)
    ...
    def _validate(
        self, response: requests.Response, request_context: RequestContext
    ) -> None:
        if response.ok:
            return
